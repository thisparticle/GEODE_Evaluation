...
def single_test(start_idx, end_idx):
    for idx in range(start_idx, end_idx + 1):
        ...
        command = 'bash {}'.format(seq_main_name)
        os.system(command)
        command = 'cp /home/thisparticle/baseline_ws/fast-lio_ws/src/FAST_LIO/Log/pos_log.txt {}'.format(os.environ['result_path'])
        # os.system(command)               
...
def mc_test(start_idx, end_idx, mc_trials):
    for idx in range(start_idx, end_idx + 1):
        ...
        for trial in range(0, mc_trials):
            ...
            # os.environ['data_path'] = '/data/bags/S3E-Degeneration-Dataset/{}/{}.bag'.format(sequence_folder, seq_name[idx])
            # os.environ['data_path'] = '/data/bags/GEODE/Metro_Tunnels/{}.bag'.format(seq_name[idx])
            os.environ['data_path'] = '/data/bags/kittti/{}.bag'.format(seq_name[idx])
            command = 'bash {}'.format(seq_main_name)
            os.system(command)        
            # command = 'mv $result_path/traj/stamped_groundtruth.txt $result_path/traj/stamped_groundtruth{}.txt'.format(trial)
            command = 'mv /home/thisparticle/baseline_ws/fast-lio_ws/src/FAST_LIO/Log/pos_log.txt $result_path/fast_lio_{}.txt'.format(trial)
            os.system(command)
            command = 'mv /home/thisparticle/baseline_ws/fast-lio_ws/src/FAST_LIO/Log/innovation.txt $result_path/innovation_{}.txt'.format(trial)
            os.system(command)
            command = 'mv /home/thisparticle/baseline_ws/fast-lio_ws/src/FAST_LIO/Log/innovation_cov.txt $result_path/innovation_cov_{}.txt'.format(trial)
            os.system(command)
            command = 'mv /home/thisparticle/baseline_ws/fast-lio_ws/src/FAST_LIO/Log/dx.txt $result_path/dx_{}.txt'.format(trial)
            os.system(command)
            command = 'mv /home/thisparticle/baseline_ws/fast-lio_ws/src/FAST_LIO/Log/error.txt $result_path/error_{}.txt'.format(trial)
            os.system(command)
...
if __name__ == '__main__':
    ...
    if args.sequence == 'Inland_Waterways_Alpha':
        ...
        seq_main_name = 'bob_main.sh'
        yaml_name = 'config_realvehicle_oxford.yaml'
    elif args.sequence == 'metro_tunnel_alpha':
        sequence_folder = 'metro_tunnel'
        ...
        seq_main_name = 'carol_main.sh'
        yaml_name = 'config_realvehicle_oxford.yaml' 
    elif args.sequence == 's3e3':
        sequence_folder = 's3e3'
        seq_name = ['Tunneling_tunnel3_alpha','Tunneling_tunnel4_alpha','Tunneling_tunnel5_alpha']
        platform = 'real_vehicle/oxford'
        seq_main_name = 's3e.sh'
        yaml_name = 'config_realvehicle_oxford.yaml'
    elif args.sequence == 'kitti':
        sequence_folder = 'kitti'
        seq_name = ['kitti_sequence_01','kitti_sequence_04','kitti_sequence_05','kitti_sequence_07','kitti_sequence_09','kitti_sequence_10']
        platform = 'real_vehicle/oxford'
        seq_main_name = 'kitti_main.sh'
        yaml_name = 'config_realvehicle_oxford.yaml' 
