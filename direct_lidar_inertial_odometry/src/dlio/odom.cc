...
void dlio::OdomNode::callbackPointCloud(const sensor_msgs::PointCloud2ConstPtr& pc) {
  ...
  // 保存轨迹为nav_msgs::Path形式（使用lidarPose）
  {
    geometry_msgs::PoseStamped pose_stamped;
    pose_stamped.header.stamp = this->scan_header_stamp;
    pose_stamped.header.frame_id = this->odom_frame;
    pose_stamped.pose.position.x = this->lidarPose.p[0];
    pose_stamped.pose.position.y = this->lidarPose.p[1];
    pose_stamped.pose.position.z = this->lidarPose.p[2];
    pose_stamped.pose.orientation.x = this->lidarPose.q.x();
    pose_stamped.pose.orientation.y = this->lidarPose.q.y();
    pose_stamped.pose.orientation.z = this->lidarPose.q.z();
    pose_stamped.pose.orientation.w = this->lidarPose.q.w();
    this->globalPath.poses.push_back(pose_stamped);
  }
 ...
 * save path
*/
void dlio::OdomNode::savePath() {
  std::ofstream file("/home/thisparticle/baseline_ws/dlio_ws/path.txt");
  file.setf(std::ios::scientific, std::ios::floatfield);
  int pathSize = globalPath.poses.size();

  for(int i = 0; i < pathSize; i++) {
    geometry_msgs::PoseStamped globalpose = globalPath.poses[i];
    // tum格式
    file << globalpose.header.stamp << " "
      << globalpose.pose.position.x << " "
      << globalpose.pose.position.y << " "
      << globalpose.pose.position.z << " "
      << globalpose.pose.orientation.x << " "
      << globalpose.pose.orientation.y << " "
      << globalpose.pose.orientation.z << " "
      << globalpose.pose.orientation.w
      << std::endl;
  }
  file.close();
}
