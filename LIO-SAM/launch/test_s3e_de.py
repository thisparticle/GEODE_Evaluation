...
def single_test(start_idx, end_idx):
    for idx in range(start_idx, end_idx + 1):
        ...
        os.environ['rpg_path'] = '/home/thisparticle/shige/S3E--/evluation/{}/LIO_SAM'.format(sequence_folder)
        os.environ['result_path'] = '{}/{}/'.format(os.environ['rpg_path'], seq_name[idx])
        command = 'mkdir -p $result_path'
        os.system(command)
        command = 'bash {}'.format(seq_main_name)
        os.system(command)
        command = 'cp /home/thisparticle/baseline_ws/lio-sam_ws/src/LIO-SAM/pathfile/lio_sam.txt {}'.format(os.environ['result_path'])
        # os.system(command)               
...
def mc_test(start_idx, end_idx, mc_trials):
    for idx in range(start_idx, end_idx + 1):
        ...
        for trial in range(0, mc_trials):
            ...
            command = 'mv /home/thisparticle/baseline_ws/lio-sam_ws/src/LIO-SAM/pathfile/lio_sam.txt $result_path/lio_sam_{}.txt'.format(trial)
            os.system(command)
